...
function Compkiller.new(Config : Window)
	...
	do
		...
		WindowArgs.KeybindConnection = UserInputService.InputBegan:Connect(function(Input,Typing)
	...
	function WindowArgs:Destroy()
		...
		if WindowArgs.KeybindConnection then
			WindowArgs.KeybindConnection:Disconnect();
			WindowArgs.KeybindConnection = nil;
		end;

		-- Stop all threads
		if WindowArgs.THREADS then
			for i, thread in next, WindowArgs.THREADS do
				if typeof(thread) == "thread" then
					task.cancel(thread);
				elseif typeof(thread) == "function" then
					-- If it's a function, try to call it to stop it (for coroutines)
					pcall(thread);
				end;
			end;
			WindowArgs.THREADS = {};
		...
		if WindowArgs.Tabs then
			for i, tab in next, WindowArgs.Tabs do
				if tab.Remote then
					pcall(function()
						tab.Remote:Disconnect();
					end);
				end;
			end;
		end;
		...
		-- Clean up Notify reference (it's a shared cache, so we just clear the reference)
		WindowArgs.Notify = nil;

		-- Clear all references
		WindowArgs.Tabs = nil;
		WindowArgs.SelectedTab = nil;
		WindowArgs.LastTab = nil;
		WindowArgs.IsOpen = nil;
		WindowArgs = nil;
	end;
